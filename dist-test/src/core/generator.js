...
function permuteDigits(grid, prng) {
    const digits = [1, 2, 3, 4, 5, 6, 7, 8, 9];
    shuffleInPlace(digits, prng);
    const mapping = new Map();
    for (let i = 0; i < 9; i++) {
        mapping.set(i + 1, digits[i]);
    }
...
function permuteRows(grid, prng) {
...
function permuteCols(grid, prng) {
...
function removalsForDifficulty(difficulty) {
