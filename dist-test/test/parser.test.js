...
const tests = [
    ...
    test('parsePuzzleString - invalid length (too short)', () => {
        const input = '123';
        assertThrows(() => parsePuzzleString(input), (err) => {
            assert(err instanceof ParseError, 'Should be ParseError');
            assertEqual(err.code, 'INVALID_LENGTH');
    ...
    test('parsePuzzleString - invalid character', () => {
        ...
        assertThrows(() => parsePuzzleString(input), (err) => {
            assert(err instanceof ParseError);
            assertEqual(err.code, 'INVALID_CHARACTER');
    ...
    test('loadPuzzleFromFile - file not found', async () => {
        ...
        await assertRejects(async () => await loadPuzzleFromFile(filePath), (err) => {
            assert(err instanceof ParseError);
            assertEqual(err.code, 'FILE_NOT_FOUND');
            return true;
        });
    }),
    test('loadPuzzleFromFile - empty file', async () => {
        ...
        await assertRejects(async () => await loadPuzzleFromFile(filePath), (err) => {
            assert(err instanceof ParseError);
            assertEqual(err.code, 'EMPTY_FILE');
            return true;
        });
    }),
