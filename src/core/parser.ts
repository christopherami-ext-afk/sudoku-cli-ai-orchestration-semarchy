...
export function parsePuzzleString(input: string): Grid {
  ...
  if (input.length !== 81) {
    throw new ParseError(
      'INVALID_LENGTH',
      `Puzzle must be exactly 81 characters, got ${input.length}`
    );
  }
  ...
  for (let i = 0; i < 81; i++) {
    ...
    if (char >= '1' && char <= '9') {
      ...
      throw new ParseError(
        'INVALID_CHARACTER',
        `Invalid character '${char}' at position ${i}. Allowed: 1-9, 0, or .`
      );
    }
...
export async function loadPuzzleFromFile(path: string): Promise<string> {
  ...
  try {
    ...
    if (!firstLine) {
      throw new ParseError(
        'EMPTY_FILE',
        `File is empty or contains no puzzle data: ${path}`
      );
    }
